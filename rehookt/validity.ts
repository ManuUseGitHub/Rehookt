import { IRehooktChecks, IRehooktException } from './interfaces';

const validity = (): IRehooktChecks => {
  ...
  const rehooktException = (
    exclass: IRehooktException,
    number: number,
    submessage: string) => {
  ...
  const checkFormat = function (state: any, stateName: any): boolean {
    ...
    if (definitions.length > 1) {
      definitions.forEach(e => {
        ...
        if (/REHOOKT_NONE/i.test(stateName)) {
          throw rehooktException(REHOOKT_UNCONCISTENCY_EXP, 3,
            'Special definition REHOOKT_NONE cannot coexist'
  ...
  return { checkAbuses, checkValidity, checkFormat, checkSpecialHooks };
};
